...
//
//   使い方:
//     node scripts/build-cm.mjs 出力.mp4 "clip.mp4:開始:終了" "clip2.mp4:開始:終了" ...
//   例:
...
if (!out || specs.length < 2) {
  console.error('使い方: node scripts/build-cm.mjs 出力.mp4 "clip.mp4:開始:終了" ...');
  process.exit(1);
}
...
parts.push(
  ...
  `drawtext=fontfile='${esc(FONT)}':text='${COPY}':fontcolor=${INK}:fontsize=58:` +
