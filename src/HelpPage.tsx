...
const CAMPFIRE_FAQS: FaqItem[] = [
  ...
  { section: "🎯 Qocca について", q: "グランドオープンはいつですか?",
    a: "2026年7月1日(火)です。このクラウドファンディング(6/1〜)で最初の住民となってくださった方は、グランドオープンと同時に「創業期住民」として街にお越しいただけます。\n\n※ 現在もQoccaは利用可能です。今すぐアカウント作成いただけます。" },

  // 🎁 リターンについて
  { section: "🎁 リターンについて", q: "「創業期住民」って何ですか?",
    a: "クラウドファンディング期間中にご支援くださった方を、私たちは「創業期住民」と呼びます。Qoccaの一番最初の住民として、バッジや特別な記念アイテム、グランドオープンへの早期招待などさまざまな特典が含まれています。これは「数量」ではなく「最初の温度」を共有してくださった証として、永久に残ります。" },
  ...
  { section: "🎁 リターンについて", q: "創業期出品者枠(¥5,000)って何ですか?",
    a: "クリエイター作家さん向けの特別リターンです。Qoccaでは作品が売れた(売買取引が完了した)際にのみ手数料が発生する仕組みで、通常は取引完了時の手数料が10%ですが、創業期出品者枠を確保された方は、1年間 5%の優遇手数料となります。\n(ウェルカムキャンペーン期間中 2026/6/1〜7/31は、全員 0%の特別措置)\n50枠限定で、創業期出品者バッジも付与されます。\n\n※ 出品自体は無料です。\n※ 作品が売れた時のみ、手数料が発生します。" },
  { section: "🎁 リターンについて", q: "ARK募金リターンって何ですか?",
    a: "¥3,000のリターンには、¥2,000がARK(Animal Refuge Kansai)へ寄付される仕組みが含まれています。ARKは1990年から活動されている日本最大級の動物保護施設です。Qoccaは売上の3%もARKへ寄付しており、ARK募金リターンは「Qoccaの応援 + 動物保護への直接貢献」を同時にできる特別なリターンです。\n\n※ARKとの連携を運用ルールに沿って進めています。" },
  { section: "🎁 リターンについて", q: "創業パートナー(¥100,000)とは?",
    a: "Qoccaの歴史に「創業パートナー」として名前を刻むリターンです。利用規約に明記され、Qoccaのトップページに永久掲載、運営代表(大木 政和)との1時間個別ミーティング、ロードマップへの意見反映機会などが含まれます。5枠限定で、Qoccaの未来を一緒に作っていただく方向けです。" },
  ...
  { section: "🎨 クリエイターについて", q: "出品するにはどうすればいいですか?",
    a: "すでに今すぐ、qocca.pet にアカウントを作成して出品いただけます! 出品自体は完全無料です。作品が売れた(売買取引が完了した)時のみ手数料が発生する仕組みです。ウェルカムキャンペーン期間 (2026/6/1〜7/31)は全員手数料無料、創業期出品者枠(¥5,000)の方は1年間 取引完了時の手数料が5%の特別優遇があります。" },
...
const CampfireFaqGuide: React.FC = () => {
  const navigate = useNavigate();
  // section ごとにグループ化 + 連番付与
  const groupedSections: Array<{ section: string; items: Array<FaqItem & { idx: number }> }> = [];
  let runningIdx = 0;
  for (const faq of CAMPFIRE_FAQS) {
    runningIdx += 1;
    const existing = groupedSections.find(g => g.section === faq.section);
    if (existing) {
      existing.items.push({ ...faq, idx: runningIdx });
    } else {
      groupedSections.push({ section: faq.section, items: [{ ...faq, idx: runningIdx }] });
    }
  }
  ...
  return (
    <PageWrap>
      ...
      <Note color="blue">
        Qoccaのクラウドファンディングについての疑問に、ひとつずつ静かにお答えします。<br/>
        ここに無い質問があれば、お気軽にお問い合わせください。
      </Note>
        ...
        <div key={group.section} style={{ marginBottom: 32 }}>
          ...
          {group.items.map(faq => (
            <div key={faq.idx} style={{ marginBottom: 18, paddingBottom: 14, borderBottom: `1px solid ${C.border}` }}>
              ...
              <p style={{ fontSize: 13, color: "#444", lineHeight: 1.9, whiteSpace: "pre-wrap", margin: 0 }}>
                {faq.a}
              </p>
            </div>
          ))}
      ...
      <div style={{ marginTop: 24, textAlign: "center" }}>
        <button
          ...
          style={{ padding: "12px 28px", background: C.white, border: `1.5px solid ${C.orange}`, borderRadius: 22, color: C.orange, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}
        >
          お問い合わせはこちら →
        </button>
      </div>
...
const HelpPage: React.FC = () => {
  ...
  if (slug === "campfire-faq") return <CampfireFaqGuide />;
