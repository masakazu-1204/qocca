        ...
        <div style={{ background:C.white, borderRadius:20, padding:"24px 16px", border:`1px solid ${C.border}` }}>
          ...
          {showReset ? (
            ...
            <>
              ...
              <button onClick={()=>handleOAuth("twitter")} style={{
                width:"100%", padding:"11px", marginBottom:8, border:`1.5px solid ${C.border}`,
                borderRadius:10, background:C.white, cursor:"pointer", fontSize:13, fontWeight:700,
                fontFamily:"inherit", color:C.dark, display:"flex", alignItems:"center", justifyContent:"center", gap:8
              }}>
...
const MyPage = ({ setPage }) => {
  ...
  const providerLabel = provider === "google" ? "Google" : provider === "twitter" ? "X（Twitter）" : "メール";
  ...
  const pages = {
    ...
    privacy: {
      ...
      sections: [
        ...
        { h:"9. お問い合わせ", p:"プライバシーに関するお問い合わせは、アプリ内サポートまたは support@qocca.jp までご連絡ください。" },
      ]
    },
    tokusho: {
      title: "特定商取引法に基づく表記",
      ...
      sections: [
        { h:"事業者名", p:"Qocca（個人事業）" },
        { h:"代表者", p:"運営者名（開業届提出後に記載）" },
        { h:"所在地", p:"大阪府（詳細住所は請求があった場合に遅滞なく開示）" },
        { h:"連絡先", p:"support@qocca.jp（お問い合わせはアプリ内サポートをご利用ください）" },
        ...
        { h:"返品・キャンセル", p:"作業開始前（購入者都合）：決済手数料（3.6%）を差し引いた金額を返金。作業開始前（出品者都合）：全額返金。納品後72時間以内：異議申し立て可能（出品者都合の場合は全額返金、購入者に過失がある場合は手数料控除）。納品後72時間経過：取引完了（返金不可）。詳細は利用規約第7条をご確認ください。" },
        { h:"動作環境", p:"Google Chrome、Safari、Firefox、Edgeの最新版を推奨。スマートフォンはiOS 15以降、Android 10以降を推奨。" },
    ...
    contact: {
      ...
      sections: [
        ...
        { h:"メール", p:"support@qocca.jp 宛にお送りください。件名に「お問い合わせ」と注文番号（お持ちの場合）をご記入ください。" },
    ...
    <div style={{ paddingTop:60, minHeight:"100vh", background:C.cream }}>
      ...
      <div style={{ maxWidth:640, margin:"0 auto", padding:"24px 16px 80px" }}>
        ...
        <div style={{ borderTop:`1px solid ${C.border}`, paddingTop:16, marginTop:24, fontSize:11, color:C.warmGray }}>
          © 2026 Qocca Inc. All rights reserved.
        </div>
      </div>
    </div>
  );
