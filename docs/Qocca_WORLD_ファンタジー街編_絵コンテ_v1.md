...
## 2026/9/25 派生: 「街を歩くトップ」の UI 試作

King の相談「静けさは大事やけど、普通でテンションが上がらない」→ 動画で作った街に、そのまま住めるサイトにする。
試作 (Artifact・本番は触っていない): https://claude.ai/artifact/EzifCA8LCyiHG3R5WezwnZ  素材とソースは `ads-out/world-proto/`

```
構成      入口 (アーチ) → 街を歩く: 商店街 / 散歩道と案内所 / 広場 / 展示場 → 掲示板 (今日の街の数字) → 夕方の広場で「この街の名前は、Qocca」
光        実際の時刻で 朝/昼/夕/夜 に色と提灯の灯りが変わる (右上で手動切替も可)
動き      スクロールに少し遅れて風景が動く (歩く感じ)・光の粒・落ち葉。動物は動かさない。prefers-reduced-motion で全部止まる
守ったもの 絵文字なし・オレンジなし・font-weight ≤500・transition ≥0.6s・QC トークン・Zen Kaku + Instrument Serif
本番化    King が「これや」なら、home.tsx のヒーロー → 区画 の順に移す (デザイン刷新は King マージ)。商店街・マイページの旧ブランド色も揃える
```
