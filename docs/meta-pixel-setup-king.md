...
> 📌 **フル ID 明示について** (2026/6/11 #143 後追い更新): Meta Pixel ID は **HTML / bundle.js に埋め込まれ誰でも閲覧できる公開識別子** (= token/secret ではない) のため、引き継ぎ時の取り違え防止を優先して**フル値を記載**する。(#139 では「末尾4桁のみ」方針だったが、引き継ぎ書での混同防止を優先しフル明示に更新。)
...
- 2026/6/11 (依頼書 #143 後追い) v1.2: 引き継ぎ混同防止のため Pixel ID をフル値 (`1039178921791385`) で明示 (公開識別子のため秘匿不要 / #139 の末尾4桁方針を更新)
