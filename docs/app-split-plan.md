...
| Phase 2 | ✅ 完了 | (このコミット) | `supabase` は #119 で既に `supabaseClient.ts` へ外出し済。本Phaseで `AuthContext`/`AuthProvider`/`useAuth` を `contexts/AuthContext.tsx` へ移動 (App.tsx −104行)。⚠️ `createContext` は複製せず移動のみ＝`createContext(null)` は AuthContext.tsx の1箇所だけ。App.tsx の React import から未使用化した `createContext`/`useContext` を除去。vite緑 + tsc緑 + ブラウザ実描画(ホーム全描画・DB実データ・useAuth解決・未ログイン状態正常)で検証。<br>📌 別件メモ: 「Multiple GoTrueClient」警告は**元からある構造負債**(componentに createClient が計12箇所)。本Phaseとは無関係。将来 supabaseClient.ts 単一統合の別Phase候補 |
| Phase 3 | ⬜ 未着手 | — | |
| Phase 4 | ⬜ 未着手 | — | |
| Phase 5 | ⬜ 未着手 | — | |
| Phase 6 | ⬜ 未着手 | — | |
