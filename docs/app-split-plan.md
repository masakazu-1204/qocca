...
| Phase 6 | 🟡 進行中 (6a ✅ main済 / 6b deep-dive中) | 6a: e93d753(PR#11) | **6a 完了・main マージ済 (2026-06-15, PR #11 squash)**: 決済導線に触れない SearchPage / UserProfilePage → `pages/marketplace.tsx`(App.tsx −508行)。循環2件回避(SignupPage→`<MyPage>`Phase7残留=Phase7同梱 / DetailPageWrapper→`<DetailPage>`6b=6b同梱)。死にimport掃除(calcPopularityScore+sortByPopularity)。検証=実描画(/search /user authガード)エラー皆無。<br>**6b (決済本丸) deep-dive中**: DetailPage(2985-3856)/SellPage(3857-4722)/submitListing(65)/DetailPageWrapper(10588) → marketplace.tsx 予定。⚠️地雷=DetailPage が `<ListingEditModal>`(8236, Phase7) 参照→neutralize要検討。移動前にdeep-dive(決済文字列/BP計算/購入確認モーダルJSX完全保全/create-checkout/SUPABASE_URL精査)完了→報告→GO の流れ。 |
| Phase 7 | ⬜ 未着手 | — | |
| Phase 8 | ⬜ 未着手 | — | |
